...
const KeyboardShortcut = ({ children }: { children: React.ReactNode }) => {
  return (
    <span className="bg-cyan-800 text-cyan-400 border-cyan-500 border rounded px-2 py-1 me-1">
      {children}
    </span>
  );
};
...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="flex flex-col min-h-screen bg-cyan-950 text-white w-screen">
      ...
      <main className="flex flex-col items-center justify-center min-h-[calc(100vh-6em)] px-20 text-center bg-linear-to-br  from-cyan-950 to-sky-950">
        {children}
      </main>
      <footer className="flex flex-col items-center justify-center h-2 w-full px-20 text-center">
        <div className="text-xs mt-8 flex flex-row gap-5 items-start justify-end">
          ...
          <div>
            <KeyboardShortcut>Ctrl+C/V</KeyboardShortcut> copy/paste selected
            cells
          </div>
          <div>
            <KeyboardShortcut>Ctrl+Z</KeyboardShortcut> undo
          </div>
          <div>
